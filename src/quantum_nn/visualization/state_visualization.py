...
def plot_state_bloch(
    ...
    if qubit_indices is None:
        qubit_indices = list(range(min(n_qubits, 6)))  # Limit to 6 qubits for readability
...
def plot_state_city(
    ...
    if amplitudes.shape[0] <= 16 and amplitudes.shape[1] <= 16:
        ...
        y_labels = [format(i, f"0{n_qubits-n_qubits//2}b") for i in range(amplitudes.shape[0])]
