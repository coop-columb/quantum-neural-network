...
class BenchmarkRunner:
    ...
    def _plot_performance_summary(self, results: Dict[str, Any], run_dir: str):
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(run_dir, "performance_summary.png"), bbox_inches="tight")
        plt.close(fig)
    ...
    def _generate_comparison_visualizations(
        ...
        comparison_dir = os.path.join(self.output_dir, f"comparison_{task_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def _plot_performance_comparison(
        self, results: Dict[str, Dict[str, Any]], model_names: List[str], output_dir: str
    ):
    ...
    def _plot_timing_comparison(
        ...
        # Create figure with a subplot for each metric
        fig, axes = plt.subplots(
            len(timing_metrics), 1, 
            figsize=(10, 4 * len(timing_metrics)),
            sharex=True
        )
        ...
        # Adjust layout and save
        plt.tight_layout()
        fig.savefig(os.path.join(output_dir, "timing_comparison.png"), bbox_inches="tight")
        plt.close(fig)
