...
def get_benchmark_task(
    ...
    if task_name == "binary_classification":
        ...
        data = _generate_multiclass_classification(
            n_samples, random_state, **task_kwargs
        )
        ...
        data = _generate_entanglement_classification(
            n_samples, random_state, **task_kwargs
        )
    else:
...
def _generate_quantum_simulation(
    ...
    for i in range(n_samples):
        ...
        for j in range(n_qubits):
            ...
            y[i, j] = np.cos(rx) * np.cos(ry) * np.cos(rz) + np.sin(rx) * np.sin(
                ry
            ) * np.sin(rz)
...
def _generate_entanglement_classification(
    ...
    task_info = {
        ...
        "n_classes": 2,
        "n_features": X.shape[1],
        "feature_names": [f"param_{i}" for i in range(X.shape[1] - 1)]
        + ["entangling_param"],
