...
class DatasetDownloader:
    ...
    def extract_dataset(self, zip_path: Optional[str] = None) -> bool:
        ...
        """
        final_zip_path = Path(zip_path) if zip_path is not None else self.zip_path

        if not final_zip_path.exists():
            print(f"Zip file not found: {final_zip_path}")
            return False

        print(f"Extracting dataset from {final_zip_path}...")
    ...
    def get_dataset_stats(self) -> dict:
        ...
        stats: Dict[str, Any] = {"dataset_path": str(self.dataset_path), "splits": {}}
