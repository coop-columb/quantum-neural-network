...
def create_synthetic_medical_data(
    ...
    for i in range(n_samples):
        ...
        if class_label == 0:
            ...
            mask = (x - center_x) ** 2 + (y - center_y) ** 2 <= (image_size[0] // 4) ** 2
            img[mask] += 0.3
            ...
            for _ in range(5):
                ...
                img[max(0, x-size):min(image_size[0], x+size), 
                    max(0, y-size):min(image_size[1], y+size)] += 0.4
...
def main():
    ...
    try:
        ...
        quantum_features, labels, train_ds, val_ds, class_weights = demonstrate_preprocessing()
