...
class DataPipeline:
    ...
    def __init__(
        ...
        logger.info(
            f"DataPipeline initialized with batch_size={batch_size}, "
    ...
    def create_datasets(
        ...
        val_dataset = self._create_tf_dataset(x_val_processed, y_val, is_training=False)

        logger.info(
            f"Created datasets - Train: {len(x_train)} samples, "
    ...
    def _create_tf_dataset(
        self, images: np.ndarray, labels: np.ndarray, is_training: bool = True
    ) -> tf.data.Dataset:
        """
    ...
    def create_quantum_datasets(
        ...
        val_dataset = self._create_tf_dataset(x_val_quantum, y_val, is_training=False)

        logger.info(
            f"Created quantum datasets - Features shape: {x_train_quantum.shape[1:]}"
        )
