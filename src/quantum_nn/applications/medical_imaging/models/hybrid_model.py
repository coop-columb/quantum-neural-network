...
class MedicalHybridModel(tf.keras.Model):
    ...
    def _build_small_model(self, inputs):
        ...
        quantum_layer = QuantumLayer(
            n_qubits=self.n_qubits, measurement_type="expectation"
        )
        x = quantum_layer(x)
    ...
    def _build_medium_model(self, inputs):
        ...
        quantum_layer2 = QuantumLayer(
            n_qubits=self.n_qubits // 2, measurement_type="expectation"
        )
        x = quantum_layer2(x)
    ...
    def _build_large_model(self, inputs):
        ...
        for i in range(3):
            ...
            quantum_layer = QuantumLayer(
                n_qubits=self.n_qubits - i * 2, measurement_type="expectation"
            )
            x = quantum_layer(x)
    ...
    def compile_for_medical_imaging(
        ...
        logger.info(
            f"Hybrid model compiled for medical imaging with {self.model_size} architecture"
        )
...
def create_hybrid_medical_model(
    ...
    **kwargs,
    ...
    logger.info(
        f"Created hybrid medical model: {model_size} size, "
